...
/**
 * MarathonGlyphs - Les vrais motifs Marathon (fig01-07)
 * Reproduits fidèlement depuis les SVG originaux
 */

// Fig01 - Colonnes connectées avec arches (style "M" Marathon)
export const GlyphColumns = memo(function GlyphColumns({
  size = 100,
  color = '#C2FE0B',
  className = '',
}) {
...
// Fig02 - Forme P/R fluide (retour)
export const GlyphReturn = memo(function GlyphReturn({
  size = 100,
  color = '#C2FE0B',
  className = '',
}) {
...
// Fig03 - Triple vagues connectées
export const GlyphWaves = memo(function GlyphWaves({
  size = 100,
  color = '#C2FE0B',
  className = '',
}) {
...
// Fig04 - Cornes / Y inversé
export const GlyphHorns = memo(function GlyphHorns({
  size = 100,
  color = '#C2FE0B',
  className = '',
}) {
...
// Fig05 - Barre + double arche
export const GlyphArches = memo(function GlyphArches({
  size = 100,
  color = '#C2FE0B',
  className = '',
}) {
...
// Fig06 - Double parallélogramme croisé
export const GlyphParallel = memo(function GlyphParallel({
  size = 100,
  color = '#C2FE0B',
  className = '',
}) {
...
// Fig07 - Hexagone barré (double forme)
export const GlyphHexSlash = memo(function GlyphHexSlash({
  size = 100,
  color = '#C2FE0B',
  className = '',
}) {
...
// Composant pour afficher tous les glyphes
export const AllGlyphs = memo(function AllGlyphs({
  size = 80,
  color = '#C2FE0B',
  gap = 24,
  className = '',
  ...
  return (
    <div className={`flex flex-wrap items-center justify-center ${className}`} style={{ gap }}>
      <GlyphColumns size={size} color={color} />
      <GlyphReturn size={size} color={color} />
      <GlyphWaves size={size} color={color} />
      <GlyphHorns size={size} color={color} />
      <GlyphArches size={size} color={color} />
      <GlyphParallel size={size} color={color} />
      <GlyphHexSlash size={size} color={color} />
    </div>
  );
});
...
export default {
  ...
  Horns: GlyphHorns,
  Arches: GlyphArches,
  Parallel: GlyphParallel,
  HexSlash: GlyphHexSlash,
  All: AllGlyphs,
};
